# --- path fix (mantém a estrutura antiga) ---
import os, sys
...
# Carrega fieldmap (IDs reais do FS)
try:
...
# Filtros de data (como no antigo)
...
# Busca: somente Core (/rest/api/3/search) com paginação (modelo antigo)
try:
...
# Montagem dos itens e filtro por data de agendamento (ou created)
items = []
for issue in issues:
    fields = issue.get("fields", {}) or {}
    # usa 'data_agendamento' do fieldmap se existir; senão cai para 'created'
...
from itertools import groupby
def date_key(row): return row[0] or dt.date.min

for day, group in groupby(items, key=date_key):
